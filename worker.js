...
const { VersionError } = require("mongoose");
  ...
  while (retryCount <= 4) {
    try {
      ...
      if (error && error.name === "VersionError") {
        retryCount++;
        const freshDomain = await Domain.findOne({});
        freshDomain.integrations.hubspot.accounts.forEach((account) => {
          const oldAccount = domain.integrations.hubspot.accounts.find(
            (account) => account.hubId === account.hubId,
          );

          account.lastPulledDates.companies = _.max(
            account.lastPulledDates.companies,
            oldAccount.lastPulledDates.companies,
          );
          account.lastPulledDates.contacts = _.max(
            account.lastPulledDates.contacts,
            oldAccount.lastPulledDates.contacts,
          );
          account.lastPulledDates.deals = _.max(
            account.lastPulledDates.deals,
            oldAccount.lastPulledDates.deals,
          );
          account.lastPulledDates.meetings = _.max(
            account.lastPulledDates.meetings,
            oldAccount.lastPulledDates.meetings,
          );
        });
      } else {
        throw error;
...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    const contacts = await hubspotClient.crm.contacts.batchApi.read({
      inputs: _.uniqBy(
        meetingAssociations.results.flatMap((result) =>
          result.to.map((to) => ({ id: to.id })),
        ),
        "id",
      ),
      properties: [
    ...
    const contactsRecord = contacts.results.reduce((acc, contact) => {
      if (contact.properties.email) {
        acc[contact.id] = contact;
      }
      return acc;
    }, {});
    const meetingsToContacts = meetingAssociations.results.reduce(
      (acc, associations) => {
        acc[associations._from.id] ??= [];

        for (const to of associations.to) {
          const contact = contactsRecord[to.id];
          if (contact) {
            acc[associations._from.id].push({
            ...
            acc[associations._from.id] = _.uniqBy(
              acc[associations._from.id],
              "id",
            );
          }
    ...
    data.forEach((meeting) => {
      ...
      const actionTemplate = {
        ...
        meetingProperties: {
          ...
          meeting_start_time: meeting.properties.hs_timestamp,
          ...
          meeting_contacts: meetingsToContacts[meeting.id] ?? [],
        },
      };
